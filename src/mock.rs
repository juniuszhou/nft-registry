...
decl_module! {
    pub struct Module<T: Trait> for enum Call where origin: T::Origin  {
        ...
        fn create_token(origin) -> DispatchResult {
            let sender = ensure_signed(origin)?;
            let nonce = Nonce::get();
            // let random_hash = (<frame_system::Module<T>>::random_seed(), &sender, nonce)
            // .using_encoded(<T as frame_system::Trait>::Hashing::hash);
            let random_hash = (T::Randomness::random_seed(), &sender, nonce).using_encoded(<T as frame_system::Trait>::Hashing::hash);
